...
class Oracle:
    ...
    def convert_relation(self, relation):
        ...
        def convert_simple_ids(relation):
            new_entities = set()
            for el in relation.entities:
                string = list(el.strings)[0]
                # print('string:' , string)
                _id = self.convert_string(string)
                # print('id: ', _id)
                if len(_id) == 1:
                    # print('simple mention or non-mention')
                    normalized_entity = el.add_entity_id(_id)
                    # print('normalized_entity: ', normalized_entity)
                    new_entities.add(normalized_entity)
                    # print('final_relation: ', relation)
                else:
                    new_entities.add(el)
        ...
        def entity_is_converted(entity):
            string = list(entity.strings)[0]
            _id = self.convert_string(string)
            if _id is not None and entity.entity_id is None:
                return False
            else:
                return True
        ...
        def explode(relation):
            ...
            def explode_inner(multi_entity, ids, relation):

                other_entities = relation.entities - {multi_entity}

                def workhorse(_id):
                    relation_copy = deepcopy(relation)
                    multi_entity_copy = deepcopy(multi_entity)

                    new_entity = multi_entity.add_entity_id(_id)
                    new_entities = other_entities | {new_entity}
                    relation_copy = replace(relation_copy, entities=new_entities)

                    return relation_copy

                return {workhorse(el) for el in ids}
            ...
            for el in relation.entities:
                if el.entity_id is None:
                    ...
                    _id = self.convert_string(string)
                    if _id is not None:
                        new_relations = explode_inner(el, _id, relation)
        ...
        while len(incomplete_relations) > 0:
            ...
            relation = convert_simple_ids(relation)
            # print('converted_relation: ', relation)
...
class DCEPositiveCombinationExampleScorer(LowercaseScorer):
    ...
    def filter_non_positive_relations(self):
        def workhorse(relations):
            return set(el for el in self.predicted_relations if el.relation_type == 'positive combination')
